...
def load_and_parse_public_key(key_bytes):
    ...
    try:
        ...
        key = serialization.load_ssh_public_key(key_bytes)
        console.log(f"Instance: {isinstance(key, ed25519.Ed25519PublicKey)}")
...
def extract_and_verify_signature(content, data_type):
    ...
    # Get public key from file input
    ...
    # Get key type from data attribute
    key_type = public_key_input.getAttribute("data-key-type")
    if not key_type:
    ...
    def verify_with_key(original_content, signature, key_type):
        def on_load(event):
            try:
                ...
                # Load the public key
                public_key, _ = load_and_parse_public_key(key_bytes)
                
                # Verify the signature
                is_valid, message = verify_signature(public_key, key_type, signature, original_content)
    ...
    try:
        ...
        if data_type == "website" or data_type == "email":
            if "?signature=" in content:
                ...
                verify_with_key(original_content, signature, key_type)
...
def handle_private_key_upload(event):
    ...
    def on_load(event):
        try:
            ...
            if key:
                document.querySelector("#key-status").textContent = f"{key_type.upper()} private key is valid and ready for use"
                document.querySelector("#signature-checkbox").disabled = False
                # Store key type as data attribute
...
def decode_qr():
    ...
    async def process_image():
        try:
            ...
            if hidden_message:
                document.querySelector("#decode-decoded-message").textContent = f"Hidden Message: {hidden_message}"
            else:
                document.querySelector("#decode-decoded-message").textContent = "No hidden message found or invalid format."
        except Exception as e:
            console.log(f"Error decoding image: {e}")
