...
def verify_qr_signature():
    ...
    async def process_verification():
        try:
            ...
            # Get signature from either steganography or file
            signature = None
            if has_sig_file:
                ...
                try:
                    ...
                    # If not hex, try to use as-is (for base64 or other encodings)
                    signature = signature_text.encode('utf-8')
            elif img_element.src:
                # Extract signature from steganography
                signature = decode_signature_from_image(img)
            
            if not signature:
                document.querySelector("#verification-result").textContent = "No signature found."
                document.querySelector("#verification-result").style.color = "red"
                return
            ...
            # Create hash object based on key type
            if key_type == 'rsa':
                ...
                # ED25519 signs the raw message directly, no separate hashing
                computed_hash = qr_content.encode('utf-8')

            # Verify signature
            try:
                if key_type == 'ed25519':
                    public_key.verify(signature, computed_hash)
                    result_message = "✓ ED25519 Signature verified! Content is authentic."
                elif key_type == 'rsa':
                    public_key.verify(
                        signature,
                        computed_hash,
                    ...
                    result_message = "✓ RSA Signature verified! Content is authentic."
                else:
                    result_message = "✗ Unsupported key type"
                
                result_element = document.querySelector("#verification-result")
                result_element.textContent = result_message
                result_element.style.color = "#4CAF50"  # Green
                
            except Exception as e:
                result_element = document.querySelector("#verification-result")
                result_element.textContent = f"✗ Signature verification failed: {str(e)}"
