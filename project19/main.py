    ...
    async def process_image():
        try:
            ...
            if code:
                ...
                # First try to get signature from the image
                signature = decode_signature_from_image(img)
                ...
                if signature:
                    ...
                    if not key_type:
                        document.querySelector("#verification-result").textContent = "Unknown key type"
                        document.querySelector("#verification-result").style.color = "red"
                        return
                    
                    # Verify signature
                    is_valid, message = verify_signature(public_key, key_type, signature, content)
                    
                    verification_result = document.querySelector("#verification-result")
                    if is_valid:
                        verification_result.textContent = "Signature verified! Content is authentic."
                        verification_result.style.color = "green"
                    else:
                        verification_result.textContent = message
                        verification_result.style.color = "red"
                else:
                    document.querySelector("#verification-result").textContent = "No digital signature found in the image."
                    document.querySelector("#verification-result").style.color = "red"
            else:
